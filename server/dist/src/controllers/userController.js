...
exports.registerUser = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (missingFields.length > 0) {
        throw new apiError_1.ApiError(400, `Missing required fields: ${missingFields.join(', ')}`);
    }
    // Validate phone if provided
    if (phone && phone.toString().trim() === '') {
        throw new apiError_1.ApiError(400, 'Phone number cannot be empty if provided');
    }
    // Check if user exists with case-insensitive email
    console.log('Checking for existing user with email:', email);
    const existingUser = await userModel_1.User.findOne({ email: { $regex: new RegExp(`^${email}$`, 'i') } });
    if (existingUser) {
        console.log('User already exists with email:', email);
        console.log('Existing user ID:', existingUser._id);
        throw new apiError_1.ApiError(409, `This email is already registered. Please use a different email or try logging in.`);
    }
